...
**GitHub Actions に採用してよいのは、公開リポジトリで無料で実行できるツールだけです。** 新規に採用する前に、以下をすべて確認し、確認できないものは採用しないでください。
...
4. Action 本体が LLM の API キーを必須とするものでないか、その Action の README / ドキュメントで確認する。キーを渡さなければ黙ってスキップする実装であっても、それは「無料で動いている」のではなく「動いていない」だけです。
