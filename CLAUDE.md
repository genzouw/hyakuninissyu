...
**GitHub Actions に採用してよいのは、公開リポジトリで無料で実行できるツールだけです。** 採用前に (1) 公式料金ページで「公開 OSS では課金が発生しない」ことを確認、(2) OSS 無料枠に審査や star 数の条件があるなら本リポジトリが現時点で満たしているかを確認、(3) 無料トライアル中であることを「無料」と判断しない、(4) その Action が LLM の API キーを必須としないことを確認、の 4 点を必ず行い、根拠 URL を PR 本文に記載してください。
