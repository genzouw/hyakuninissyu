...
さらに、意図しない機密情報（AI の作業ディレクトリ、汎用的なシークレットファイル、パッケージマネージャーの設定ファイル、DB のダンプファイル、PII データのエクスポートファイル (`*.csv`, `*.tsv`, `*.xls`, `*.xlsx`, `*.xlsm`, `*.jsonl`, `*.ndjson`)、`*.log`ファイル等の作業ログ）、ならびに各種キーストア（`*.jks`, `*.keystore`, `*.truststore`）、VPN設定（`*.ovpn`）、パスワードマネージャー（`*.kdbx`, `*.kdb`）、ネットワークキャプチャ（`*.pcap`, `*.pcapng`）、シェル履歴（`.*_history`）、および DB クライアント設定（`.dbeaver-data-sources.xml`）の漏洩を未然に防ぐため、`.pre-commit-config.yaml` にてローカル定義のカスタムフック `forbid-sensitive-files` を追加・強化しました（`repo: local` はフックの定義形式を示すもので、実行自体は `.github/workflows/pre-commit.yml` の `pre-commit run --all-files` により CI 上でも行われます）。これにより、`.gitignore` や `.gitattributes` での漏れがあった場合でも、コミットの段階でステージングを自動的にブロックし、多層的な防御をより強固にしています。
