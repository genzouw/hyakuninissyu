...
- **仕組み**: GitHub Actionsによる継続的なスキャン。
  - **対象ブランチの拡張**: `pre-commit.yml`, `gitleaks.yml`, `trivy.yml`, `trufflehog.yml`, `codeql.yml`, `osv-scanner.yml`, `actionlint.yml`, `zizmor.yml` といった主要なセキュリティスキャンワークフローは、`main` ブランチを含むすべてのブランチにマッチするワイルドカード（`**`）を使用し、**すべてのブランチへのプッシュ時**および**すべてのプルリクエスト時**にも自動実行されるよう構成されています。これにより、PR作成前であってもシークレット漏洩や脆弱性を早期に検知・ブロックします。なお `actionlint.yml` のみ `paths` フィルタ（`.github/workflows/**` 等）を併用しているため、対象ブランチは全ブランチに拡張されていますが、実行されるのは GitHub Actions 関連ファイルが変更されたプッシュ／PR に限られます。
...
- **フォーク PR からの漏洩防止**: 悪意のあるフォークリポジトリからの Pull Request (PR) 経由で、リポジトリのシークレットが漏洩したり、悪意のあるコードが強い権限で実行されたりするのを防ぐため、GitHub Actions ワークフローでは原則として `pull_request_target` トリガーを使用しません。代わりに `pull_request` を使用し、安全なコンテキストで CI が実行されるようにしています。
