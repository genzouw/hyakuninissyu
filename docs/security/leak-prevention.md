...
- **仕組み**: GitHub Actions による継続的なスキャン。
  ...
  - `trufflehog.yml`: プッシュ時および PR 時にシークレット検証を実行し、無効化済み・ローテート済みのシークレットも含めたあらゆるシークレットパターンの混入をリアルタイムにブロック。
...
- **仕組み**:
  ...
  - `trufflehog.yml` による包括的なシークレット検証。PR・Push 時のリアルタイムブロックに加え、スケジュール実行でリポジトリ全履歴に対してもシークレット検証を実施します。
...
### 新規追加: IDEワークスペースおよびOS依存ファイルの漏洩防止強化

新たに、IDE 固有の設定ファイル（`.idea/`、`.vscode/launch.json`等）や OS 自動生成ファイル（`.DS_Store`等）について、他のツールと同様に `.gitignore`, `.gitattributes`, `.vscode/settings.json`, および `.pre-commit-config.yaml` を用いて、意図しないステージングや diff 出力を防ぐ設定を追加しました。これにより、デバッグ用のローカル環境変数や一時的なシークレットが意図せずコミットされるリスクを未然に防止します。

### 新規追加: pre-commit ローカル防御フックの厳格化

さらに、意図しない機密情報（AI の作業ディレクトリ、汎用的なシークレットファイル、パッケージマネージャーの設定ファイル、DB のダンプファイル、`*.log`ファイル等の作業ログ）の漏洩を未然に防ぐため、`.pre-commit-config.yaml` にてローカル専用のカスタムフック `forbid-sensitive-files` を追加・強化しました。これにより、`.gitignore` や `.gitattributes` での漏れがあった場合でも、コミットの段階でステージングを自動的にブロックし、多層的な防御をより強固にしています。

### 新規追加: データエクスポートファイル・エディタ一時ファイルによる個人情報 (PII) 漏洩防止強化

ユーザーデータやメールアドレス等の個人情報 (PII) が含まれやすいスプレッドシートやデータエクスポートファイル（`*.csv`, `*.tsv`, `*.xls`, `*.xlsx`, `*.jsonl`）およびエディタの一時ファイル・バックアップ（`*.swp`, `*.swo`, `*~`）についても、他のファイルと同様に `.gitignore`, `.gitattributes`（`-diff`）, および `.vscode/settings.json` を用いて、意図しないステージングや diff 出力を防ぐ設定を追加しました。`.pre-commit-config.yaml` のローカル専用カスタムフック `forbid-sensitive-files` にもこれらの拡張子を追加し、誤ってテストデータや本番データがパブリックリポジトリにコミットされるリスクを多層的にブロックしています。
