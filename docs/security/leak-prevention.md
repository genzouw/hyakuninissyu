...
プロジェクトの標準パッケージマネージャー (Bun) 以外の異種ロックファイル (`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`) について、コミットやステージングをブロックする `forbid-foreign-lockfiles` カスタムローカルフックを `.pre-commit-config.yaml` に追加しました。あわせて `.gitignore` 等での除外も徹底しています。
