...
- **仕組み**: GitHub Actionsによる継続的なスキャン。
  - **対象ブランチの拡張**: `pre-commit.yml`, `gitleaks.yml`, `trivy.yml`, `trufflehog.yml` といった主要なセキュリティスキャンワークフローは、`main` ブランチのみならず、**フィーチャーブランチを含むすべてのブランチ**（`**`）**へのプッシュ時**および**すべてのプルリクエスト時**にも自動実行されるよう構成されています。これにより、PR作成前であってもシークレット漏洩や脆弱性を早期に検知・ブロックします。
