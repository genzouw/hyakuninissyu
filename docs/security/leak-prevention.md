...
- **仕組み**:
  ...
  - `pre-commit.yml` のスケジュール実行による、`detect-secrets` やカスタムフック（`forbid-sensitive-files`, `forbid-pull-request-target`等）を用いた定期監査。これにより、ルールの自動更新後に過去の履歴や既存ファイルに新たな違反がないかを継続的に確認します。
