...
- 再発防止のため、本リポジトリの [`CLAUDE.md`](../../CLAUDE.md) の「外部サービス / 外部 API は無料枠で利用できるものに限定 (有料プラン必須・有料 API キー必須のサービスは導入しない、提案 PR はクローズ)」というポリシーと整合するように指示プロンプトを書き直しました。
...
| AI コードレビュー       | [Qodo Merge (旧 PR-Agent)](https://github.com/apps/qodo-merge) (`.pr_agent.toml`, ホスト型 GitHub App)   | PR スコアリング / 自動要約 / `/review` 等のコメントコマンド           |
...
- 本リポジトリ固有の方針については [`CLAUDE.md`](../../CLAUDE.md)、[`.github/copilot-instructions.md`](../../.github/copilot-instructions.md)、[`CONTRIBUTING.md`](../../CONTRIBUTING.md)、[`SECURITY.md`](../../SECURITY.md) を必ず確認する。
